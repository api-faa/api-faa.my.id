...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        const apiUrl = `https://api-faa.my.id/faa/tojapanese?url=${encodeURIComponent(uploadedUrl)}`;
        const response = await axios.get(apiUrl, { responseType: 'arraybuffer' });

        await sock.sendMessage(remoteJid, { 
            image: Buffer.from(response.data), 
            caption: `_🗾 Hasil gaya Jepang sukses dibuat!_`
        }, { quoted: message });

        fs.unlinkSync(mediaPath);
    } catch (error) {
        console.error("Error in tojapanese handler:", error);
