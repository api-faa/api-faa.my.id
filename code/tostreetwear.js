...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        const apiUrl = `https://api-faa.my.id/faa/tostreetwear?url=${encodeURIComponent(uploadedUrl)}`;
        const response = await axios.get(apiUrl, { responseType: 'arraybuffer' });

        await sock.sendMessage(remoteJid, { 
            image: Buffer.from(response.data),
            caption: `✅ ɢᴀʏᴀ sᴛʀᴇᴇᴛᴡᴇᴀʀ ʙᴇʀʜᴀꜱɪʟ ᴅɪᴛᴇʀᴀᴘᴋᴀɴ`
        }, { quoted: message });

        fs.unlinkSync(mediaPath);
    } catch (error) {
        console.error("Error in tostreetwear handler:", error);
