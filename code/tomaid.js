...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        await sock.sendMessage(remoteJid, { react: { text: "🧹", key: message.key } }); // Emoji maid
        ...
        // Ganti endpoint ke tomaid
        const apiUrl = `https://api-faa.my.id/faa/tomaid?url=${encodeURIComponent(uploadedUrl)}`;
        const response = await axios.get(apiUrl, { responseType: 'arraybuffer' });

        await sock.sendMessage(remoteJid, { 
            image: Buffer.from(response.data), 
            caption: `✅ ɢᴀᴍʙᴀʀ ʙᴇʀʜᴀꜱɪʟ ᴅɪᴜʙᴀʜ ᴊᴀᴅɪ ᴍᴀɪᴅ 👩‍🦳`
        }, { quoted: message });

        fs.unlinkSync(mediaPath);
    } catch (error) {
        console.error("Error in tomaid handler:", error);
        await sock.sendMessage(remoteJid, { text: "⚠️ Gagal memproses gambar menjadi maid. Coba lagi nanti!" }, { quoted: message });
    }
}
