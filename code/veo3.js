...
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        const apiURL = `https://api-faa.my.id/faa/veo3?prompt=${encodeURIComponent(content)}`;
        const { data } = await axios.get(apiURL);

        if (!data?.status || !data?.check_url) {
            return sendMessageWithQuote(sock, remoteJid, message, '⚠️ Gagal memulai proses video.');
        }

        await sendMessageWithQuote(sock, remoteJid, message, 
            `🪄 *Permintaan diterima!*\n🎬 Sedang diproses oleh server FAA...\n💭 *Prompt:*\n> ${content}\n\n⏱️ *Estimasi waktu:* 10–20 menit.\n\nSaya akan kirim hasil otomatis setelah selesai.`);
